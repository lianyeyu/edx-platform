...
from ..dot_overrides import authenticate, EdxOAuth2Validator
...
class AuthenticateTestCase(TestCase):
    ...
    def test_authenticate_with_username(self):
        user = authenticate(username='darkhelmet', password='12345')
        self.assertEqual(
            self.user,
            user
        )
    ...
    def test_authenticate_with_email(self):
        user = authenticate(username='darkhelmet@spaceball_one.org', password='12345')
        self.assertEqual(
            self.user,
            user
        )
